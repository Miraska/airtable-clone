...
const createRequest = async (data) => {
  const { rows } = await db.query(
    ...
    [
      ...
      data.accred_commission,
      data.exchange_rate,
      data.hidden_rate,
      data.rate_fixation_date,
      data.request_by_rate,
      data.agent_fee,
      data.actual_fee,
      data.total_amount,
      data.with_accreditor,
      data.primary_documents_received,
      data.invoice_issued,
      data.agent_signed,
      data.registered_in_bank,
      data.letter_of_credit_opened,
      data.currency_paid,
      data.letter_of_credit_released,
      data.report_signed,
      data.transaction_closed,
      data.payment_purpose,
      data.subagent,
      data.subagent_payer,
      data.sequential_number,
      data.agent_subagent_docs_prepared,
      data.swift_received,
      data.swift_status,
      data.request_link,
      data.invoice_link,
...
// Перезапись всех данных в базе данных с учетом зависимостей
const resetDatabase = async () => {
  try {
    await db.query("DELETE FROM requests")
    // // Сначала удаляем зависимости из промежуточных таблиц
    // await db.query("DELETE FROM managers_requests");
    // await db.query("DELETE FROM contractors_requests");
    // await db.query("DELETE FROM agents_requests");
    // await db.query("DELETE FROM clients_requests");
    // await db.query("DELETE FROM subagents_requests");
    // await db.query("DELETE FROM countries_requests");

    // // Удаляем данные из основного набора таблиц
    // await db.query("DELETE FROM requests");
    ...
    // await db.query("DELETE FROM subagents");
