...
interface SubagentPayerFormData {
  name: string;
}

interface SubagentPayerFormProps {
  initialData?: any;
  onSubmit: (data: SubagentPayerFormData) => void;
  onCancel: () => void;
}

export function SubagentPayerForm({ initialData, onSubmit, onCancel }: SubagentPayerFormProps) {
  const { register, handleSubmit } = useForm<SubagentPayerFormData>({
    defaultValues: {
      name: initialData?.['Наименование'] || ''
    }
  });

  const handleFormSubmit = (data: SubagentPayerFormData) => {
    onSubmit({
